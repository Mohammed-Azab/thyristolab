...
%
% Notes:
% - Half-wave rectifier conducts only during positive half-cycles (0 to π)
% - Uses single SCR with one drop Vt per cycle
% - Ideal R-load average: Vdc = (Vm/(2*pi))*(1+cos(alpha))
% - Lower average output compared to full-wave configurations
...
for k = 1:na
    ...
    % Half-wave switches once per cycle
...
if enablePlots
    ...
    gate_demo = (theta >= a_demo) & (theta <= pi);
    v_conv_demo = V_source - Vt;
    v_conv_demo(v_conv_demo < 0) = 0;
    ...
    % Figure 1: Voltages
    figure('Name', 'Half-Wave Rectifier - Voltages');
    t_layout1 = tiledlayout(2,2,'TileSpacing','compact','Padding','compact');
    title(t_layout1,sprintf('Half-Wave Rectifier Voltages (\\alpha = %.0f°, Vm=%.1fV)', alpha, Vm));
    
    nexttile; 
    plot(t_ms, V_source, 'b--', t_ms, Vbat*ones(size(t_ms)), 'g:', t_ms, v_out_demo, 'b-','LineWidth',1.2); 
    grid on; xlabel('Time (ms)'); ylabel('Voltage (V)'); 
    legend('Source V_{rect}', 'V_{bat}', 'V_{out}', 'Location', 'best');
    ...
    % Figure 2: Currents
    figure('Name', 'Half-Wave Rectifier - Currents');
    t_layout2 = tiledlayout(2,2,'TileSpacing','compact','Padding','compact');
    title(t_layout2,sprintf('Half-Wave Rectifier Currents (\\alpha = %.0f°)', alpha));
...
% Main output plot: Firing Angle vs Charging Time
...
figure('Name', 'Half-Wave Rectifier - Power Losses vs Firing Angle');
